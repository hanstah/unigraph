...
declare global {
  interface Window {
    ...
    highlightAnnotationByPosition?: (
      id: string,
      start: number,
      end: number,
      options: any
    ) => boolean;
    getTextContentPreview?: (length?: number) => {
      length: number;
      preview: string;
      wordCount: number;
    };
    findTextPosition?: (text: string) => {
      found: boolean;
...
export function debugAnnotationHighlighting() {
  ...
  console.log(
    "  - highlightAnnotationByPosition:",
    typeof window.highlightAnnotationByPosition
  );
  console.log(
    "  - getTextContentPreview:",
    typeof window.getTextContentPreview
  );
  ...
  if (typeof window.highlightAnnotation === "function") {
    ...
    const result = window.highlightAnnotation(testId, testText, {
      ...
      highlightStyle:
        "background-color: #ff9999; border: 2px solid red; padding: 2px;",
    });
 ...
 */
export function testSpecificHighlight(
  searchText: string,
  usePosition = false,
  startPos?: number,
  endPos?: number
  ...
  if (
    ...
    if (typeof window.highlightAnnotationByPosition === "function") {
      result = window.highlightAnnotationByPosition(
        annotationId,
        startPos,
        endPos,
        {
          ...
          highlightStyle:
            "background-color: #99ff99; border: 2px solid green; padding: 2px;",
        }
      );
    }
  } else {
    ...
    if (typeof window.highlightAnnotation === "function") {
      result = window.highlightAnnotation(annotationId, searchText, {
        ...
        highlightStyle:
          "background-color: #9999ff; border: 2px solid blue; padding: 2px;",
      });
    }
  }
...
export function cleanupTestHighlights() {
  const testHighlights = document.querySelectorAll(
    ".debug-highlight, .test-position-highlight, .test-text-highlight"
  );
