...
const ForceGraph3DView: React.FC<ForceGraph3DViewProps> = ({ 
  ...
  const [ownInstance, setOwnInstance] = useState<ForceGraph3DInstance | null>(null);
      ...
      try {
        ...
        if (useMainInstance) {
          ...
          if (isMainForceGraphInstanceAvailable()) {
            ...
            console.log("Got main ForceGraph3D instance:", !!forceGraphInstance);
            ...
            if (forceGraphInstance) {
              ...
              if (canvas && containerRef.current) {
                ...
                const syncCanvas = () => {
                  if (canvas && canvasClone && containerRef.current?.contains(canvasClone)) {
                    const context = canvasClone.getContext('2d');
                    if (context) {
                      context.clearRect(0, 0, canvasClone.width, canvasClone.height);
                      context.drawImage(canvas, 0, 0, canvasClone.width, canvasClone.height);
                    }
                  }
                };
                ...
                console.log("Main instance setup complete, marking as initialized");
            ...
            console.warn("Main ForceGraph3D instance not available, falling back to creating own instance");
            ...
            console.log("Creating fallback instance with scene graph:", sceneGraph.getMetadata().name);
          ...
          console.log("Creating new ForceGraph3D instance for tab (useMainInstance=false)");
          const sceneGraph = getCurrentSceneGraph();
          ...
          console.log("Creating tab instance with scene graph:", sceneGraph.getMetadata().name);
          const newInstance = initializeTabForceGraph(
            containerRef.current!,
            sceneGraph,
            "Layout"
